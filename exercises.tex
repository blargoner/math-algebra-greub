...
\subsection*{\S~7}
\begin{rmk}
If \(E\)~is complex and \(\varphi:E_{\R}\to E_{\R}\) is \(\R\)-linear, then the anticommutator \(\acomm{i_{\R}}{\varphi}:E\to E\) defined by
\[\acomm{i_{\R}}{\varphi}=i_{\R}\after\varphi+\varphi\after i_{\R}\]
is \(\C\)-linear since it is \(\R\)-linear and
\[\acomm{i_{\R}}{\varphi}\after i_{\R}=i_{\R}\after\acomm{i_{\R}}{\varphi}\]
The commutator \(\comm{i_{\R}}{\varphi}:E\to E\) defined by
\[\comm{i_{\R}}{\varphi}=i_{\R}\after\varphi-\varphi\after i_{\R}\]
is conjugate \(\C\)-linear. Note
\[\varphi=\tfrac{1}{2i}\bigl(\,\acomm{i_{\R}}{\varphi}+\comm{i_{\R}}{\varphi}\,\bigr)\]
\end{rmk}

\begin{rmk}
If \(E,E^*\) are complex dual spaces with scalar product \(\sprod{-}{-}:E^*\times E\to\C\), then \(E_{\R},(E^*)_{\R}\) are dual with scalar product \(\sprod{-}{-}_{\R}:(E^*)_{\R}\times E_{\R}\to\R\) given by
\[\sprod{x^*}{x}_{\R}=\Re\sprod{x^*}{x}\qquad(x^*\in(E^*)_{\R},\ x\in E_{\R})\]
Indeed, clearly \(\sprod{-}{-}_{\R}\) is well-defined and \(\R\)-bilinear. If \(\sprod{x^*}{-}_{\R}=0\), then for any \(x\in E_{\R}\),
\[\Re\sprod{x^*}{x}=\sprod{x^*}{x}_{\R}=0\]
and also
\[\Im\sprod{x^*}{x}=-\sprod{x^*}{i_{\R}x}_{\R}=0\]
...
\begin{rmk}
If \(E,E^*\) and \(F,F^*\) are pairs of complex dual spaces and \(\varphi:E\to F\) and \(\varphi^*:E^*\from F^*\) are complex dual maps, then the underlying real maps \(\varphi_{\R}:E_{\R}\to F_{\R}\) and \((\varphi^*)_{\R}:E_{\R}^*\from F_{\R}^*\) are dual since
\[\sprod{(\varphi^*)_{\R}y^*}{x}_{\R}=\Re\sprod{\varphi^*y^*}{x}=\Re\sprod{y^*}{\varphi x}=\sprod{y^*}{\varphi_{\R}x}_{\R}\qquad(y^*\in F_{\R}^*,\ x\in E_{\R})\]
We write
\[\varphi_{\R}^*=(\varphi_{\R})^*=(\varphi^*)_{\R}\]
\end{rmk}

\begin{rmk}
If \(E\)~is complex with Hermitian inner product \(\iprod{-}{-}_H:E\times E\to\C\) (that is, a unitary space), and \(E_{\R}\)~has the inner product \(\iprod{-}{-}:E_{\R}\times E_{\R}\to\R\) given by
\[\iprod{x}{y}=\Re\iprod{x}{y}_H\qquad(x,y\in E_{\R})\]
then if \(\varphi:E\to E\) is skew-Hermitian, the underlying real map \(\varphi_{\R}:E_{\R}\to E_{\R}\) is skew-symmetric since
\[\adj{\varphi_{\R}}=\adj{\varphi}_{\R}=(-\varphi)_{\R}=-\varphi_{\R}\]
Therefore it makes sense to talk about \(\Psi_{E_{\R}}(\varphi_{\R})\in\bigwedge^2 E_{\R}\)---which in the book is confusingly denoted~\(\Psi_E(\varphi)\).
\end{rmk}
...
so that
\[\iprod{x}{i_{\R}y}=\Re\iprod{x}{iy}_H=\Im\iprod{x}{y}_H\]
and hence
\[\sprod{\sigma_H x}{y}=\iprod{x}{y}-i\iprod{x}{i_{\R}y}\]
This shows that the map \(\sigma: E_{\R}\to E^*\) in the book is given by \(\sigma=\iota_{\R,\C}\after(\sigma_H)_{\R}\), where \((\sigma_H)_{\R}:E_{\R}\to(E^*)_{\R}\) is the underlying real map of the Riesz isomorphism and \(\iota_{\R,\C}:(E^*)_{\R}\to E^*\) is the identity map.  Note (8.16)~follows from conjugate-linearity of~\(\sigma_H\).
\end{rmk}
