...
By non-degeneracy of the scalar product, (4)~implies
...
where \(\Cayley(\varphi)\)~denotes the \emph{Cayley transform} of~\(\varphi\). If we define
...
So \(\tau\)~is an intrinsic, coordinate-free, geometric version of the Cayley transform operating on graphs in the space \(E^*\dsum E\), which carries a natural geometric structure.
\end{rmk}

\begin{rmk}
There is another inner product\footnote{See subsection~1.25.} in \(E^*\dsum E\) which is induced by the inner product in~\(E\):
...
\[\sprod{x^*}{x}_{\R}=\Re\sprod{x^*}{x}\qquad(x^*\in\ur{(E^*)},\ x\in\ur{E})\]
...
\[\sprod{\ur{(\varphi^*)}y^*}{x}_{\R}=\Re\sprod{\varphi^*y^*}{x}=\Re\sprod{y^*}{\varphi x}=\sprod{y^*}{\ur{\varphi}x}_{\R}\qquad(y^*\in\ur{F}^*,\ x\in\ur{E})\]
We write
...
\[\iprod{x}{y}=\Re\iprod{x}{y}_H\qquad(x,y\in\ur{E})\]
...
\[\sprod{\sigma_H x}{y}=\conj{\iprod{x}{y}_H}\qquad(x,y\in E)\]
For \(x,y\in\ur{E}\),
...
so \(\ur{(\sigma_H)}\)~is also the Riesz isomorphism for the underlying real spaces. Finally, note (8.16)~follows from conjugate-linearity of~\(\sigma_H\).
\end{rmk}
