...
and similarly for more than two transformations. Then \(T_E\)~becomes an algebra isomorphism from the mixed exterior algebra \(\medeprod(E^*,E)\) to a box product algebra on~\(L(\medeprod E;\medeprod E)\). The box product is associative and unital, and is \emph{commutative} on the diagonal subalgebra of~\(L(\medeprod E;\medeprod E)\).
...
\dTo<{\mu(z)}	&					&\dTo>{\mu_{\bprod}(T_E(z))}&&&\dTo<{i(z)}		&					&\dTo>{i_{\bprod}(T_E(z))}\\
...
% TODO: mixed intersection algebra
