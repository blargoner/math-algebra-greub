...
\[\acomm{i_{\R}}{\varphi}\after i_{\R}=i_{\R}\after\varphi\after i_{\R}-\varphi=i_{\R}\after\acomm{i_{\R}}{\varphi}\]
Note
\[\varphi=-\tfrac{i}{2}\bigl(\,\acomm{i_{\R}}{\varphi}+\comm{i_{\R}}{\varphi}\,\bigr)\]
where
\[\comm{i_{\R}}{\varphi}=i_{\R}\after\varphi-\varphi\after i_{\R}\]
is the commutator, which is conjugate \(\C\)-linear on~\(E\).
\end{rmk}

\begin{rmk}
If \(E\)~is complex with Hermitian inner product \(\iprod{-}{-}_H:E\times E\to\C\) (that is, a unitary space), and \(E_{\R}\)~is the underlying real space with inner product \(\iprod{-}{-}:E_{\R}\times E_{\R}\to\R\) given by
...
\[\iprod{\varphi_{\R}x}{y}=\Re\iprod{\varphi x}{y}_H=\Re\iprod{x}{-\varphi y}_H=\iprod{x}{-\varphi_{\R}y}\qquad(x,y\in E_{\R})\]
Therefore it makes sense to talk about \(\Psi_{E_{\R}}(\varphi)=\Psi_{E_{\R}}(\varphi_{\R})\in\bigwedge^2 E_{\R}\)---which in the book is confusingly denoted~\(\Psi_E(\varphi)\).
\end{rmk}
...
where \(\sprod{-}{-}:E^*\times E\to\C\) is the scalar product. For \(x,y\in E_{\R}\),
