...
\newcommand{\sssprod}[2]{\langle\!\langle\!\langle#1,#2\rangle\!\rangle\!\rangle}
...
\newcommand{\smprods}[3]{\multi{\smprod}{#1}{#2}{#3}}
...
\subsection*{\S~4}
\begin{exer}[9]
Let \(E\)~be a vector space and \(E_1,F_1\) subspaces of finite codimension in decompositions
\[E_1\dsum E_2=E=F_1\dsum F_2\]
There is a decomposition \(E=H_1\dsum H_2\) with \(H_1\subseteq E_1\sect F_1\) of finite codimension and \(H_2\supseteq E_2+F_2\). Moreover
\[(H_2\sect E_1)\dsum E_2=H_2=(H_2\sect F_1)\dsum F_2\]
\end{exer}
\begin{proof}
Let \(K=E_1\sect F_1\) and \(L=E_2+F_2\). Then there is \(H_1\subseteq K\) with \(K=(K\sect L)\dsum H_1\) and
\begin{align*}
\codim H_1&=\codim K+\dim(K\sect L)\\
	&\le\codim K+\dim L\\
	&\le 2(\codim E_1+\codim F_1)<\infty
\end{align*}
Since \(H_1\sect L=0\), there is \(H_2\supseteq L\) with \(E=H_1\dsum H_2\). The rest is immediate.
\end{proof}
...
\[D(x_1,\ldots,x_n)=\sum_{j=1}^n (-1)^{j-1}\Delta_1(\,\proj{x_1},\ldots,\delete{\proj{x_j}},\ldots,\proj{x_n}\,)\mult\pi_1(x_j)\]
...
\[\boxed{\Delta(x_1,\ldots,x_n)=\sum_{j=1}^n (-1)^{j-1}\Delta_1(\,\proj{x_1},\ldots,\delete{\proj{x_j}},\ldots,\proj{x_n}\,)\mult\pi_1(x_j)}\]
...
\[\varphi(x_1,\ldots,x_n)=\sum_{j=1}^n (-1)^{n-j}\Phi(x_1,\ldots,\delete{x_j},\ldots,x_n)\mult x_j\]
Then \(\varphi\)~is skew symmetric and \(n\)-linear, so \(\varphi=\Delta a\) where \(a=\varphi(e_1,\ldots,e_n)\). This means
\[\Delta(x_1,\ldots,x_n)a=\sum_{j=1}^n (-1)^{n-j}\Phi(x_1,\ldots,\delete{x_j},\ldots,x_n)\mult x_j\tag{2}\]
...
Geometrically, this result expresses a relationship between \(n\)-dimensional oriented volume and \((n-1)\)-dimensional oriented volume. We also see from~(1) that the latter is just the former measured \emph{relative to a fixed vector}. The fact that the volume of an \(n\)-dimensional parallelepiped is equal to the product of the volume of any \((n-1)\)-dimensional ``base'' and the corresponding ``height'' (subsection 7.15) is a special case.
\end{rmk}
...
\begin{rmk}
The characteristic polynomial of a transformation is preserved under complexification, but not when inducing complex structure.

For example, let \(\varphi:\R^2\to\R^2\) be counterclockwise rotation about the origin by \(\pi/2\) radians. With respect to the standard basis over~\(\R\), the matrix of~\(\varphi\) is
\[M_{\R}(\varphi)=\begin{pmatrix}
0&1\\
-1&0
\end{pmatrix}\]
The characteristic polynomial of~\(\varphi\) and~\(M_{\R}(\varphi)\) over~\(\R\) is \(\chi_{\varphi}^{\R}=\lambda^2+1\). If we induce the canonical complex structure on~\(\R^2\), then the matrix of~\(\varphi\) with respect to the standard basis over~\(\C\) is
\[M_{\C}(\varphi)=\begin{pmatrix}i\end{pmatrix}\]
The characteristic polynomial of~\(\varphi\) and~\(M_{\C}(\varphi)\) over~\(\C\) is \(\chi_{\varphi}^{\C}=-\lambda+i\).

The same is true for the minimum polynomial.\footnote{See chapter~XIII. In this example, \(\mu_{\varphi}^{\R}=t^2+1\) while \(\mu_{\varphi}^{\C}=t-i\).}
\end{rmk}
...
where \(\Delta\)~is the normed determinant function representing the orientation in~\(E\). In other words, \(\Delta_1\)~is just the 2-dimensional oriented area function induced by the 3-dimensional oriented volume function~\(\Delta\) and the vector~\(a\). Therefore if we \emph{define} a 3-dimensional oriented volume map~\(\Phi\) in terms of~\(\Delta_1\), we should expect that \(\Phi=\Delta a\).
...
\end{proof}
\begin{rmk}
This result shows that any function measuring 2-dimensional oriented area in planes in 3-space actually measures 3-dimensional oriented volume relative to some fixed vector in the space.
\end{rmk}
...
\begin{rmk}
Let \(E\)~be an inner product space\footnote{See subsection~1.25.} and let \(\sigma:E\to E^*\) be the Riesz isomorphism. Define the isomorphism \(Q:E^*\tprod E\to E^*\tprod E\) by
\[Q(a^*\tprod a)=\sigma a\tprod\sigma^{-1}a^*\]
If \(\varphi:E\to E\) is linear, then \(\adj{\varphi}=TQT^{-1}\varphi\):
\begin{diagram}
E^*\tprod E	&\rTo^T_{\iso}	&L(E;E)\\
\dTo<Q		&				&\dTo>{\adj{(-)}}\\
E^*\tprod E	&\rTo_T^{\iso}	&L(E;E)
\end{diagram}
\end{rmk}
\begin{proof}
If \(T^{-1}(\varphi)=a^*\tprod a\), then
\[\iprod{\varphi x}{y}=\sprod{a^*}{x}\iprod{a}{y}=\iprod{\sigma^{-1}a^*}{x}\sprod{\sigma a}{y}=\iprod{x}{TQT^{-1}(\varphi)y}\]
The general case follows by multilinearity.
\end{proof}
\noindent This shows that \(Q\)~computes the adjoint tensor, so can be used to study things like symmetry and skew-symmetry.\footnote{See subsection~8.3.}
...
The functoriality condition~(5.20) is just Cauchy-Binet.
\end{rmk}
...
\begin{rmk}
The operator~\(i(h)\) preserves decomposability.
\end{rmk}
\begin{proof}
Let
\[u^*=\eprods{x^*}{1}{p}\qquad(x^*_i\in E^*)\]
If \(u^*=0\) or \(x^*_1,\ldots,x^*_p\in\oc{\gen{h}}\), then \(i(h)u^*=0\) is decomposable. Otherwise, there exist \(y^*_1,\ldots,y^*_{p-1}\in\oc{\gen{h}}\) and \(y^*_p\in E^*\) with\footnote{See the remarks in \S~26 below.}
\[u^*=(\eprods{y^*}{1}{p-1})\eprod y^*_p\]
By Corollary~III to Proposition~5.14.1 with \(F=\gen{h}\) and \(a=h\),
\[i(h)u^*=(-1)^{p-1}\eprods{y^*}{1}{p-1}\eprod i(h)y^*_p=\pm\sprod{y^*_p}{h}\eprods{y^*}{1}{p-1}\qedhere\]
\end{proof}
...
\subsection*{\S~3}
\begin{rmk}
The \(p\)-ary box product, which generalizes the \(p\)-th exterior power to more than one linear transformation, is \(p\)-linear and symmetric, so factors through the \(p\)-th symmetric power of~\(L(E;E)\):
\begin{diagram}[nohug]
L(E;E)^p&\rTo^{\bprod^p}&L(\medeprod^p E;\medeprod^p E)\\
\dTo<{\smprod^p}&\ruTo&\\
\medsmprod^p L(E;E)&&
\end{diagram}
If \(E\)~is finite-dimensional, then the induced linear map is \emph{surjective}. In fact, since \(\medsmprod^p L(E;E)\)~is spanned by elements of the form \(\smprods{\varphi}{}{}\),\footnote{See the remark in chapter~9, \S~10 below.} it follows that \(L(\medeprod^p E;\medeprod^p E)\)~is spanned by transformations of the form
\[\underbrace{\bprods{\varphi}{}{}}_p=p!\,\medeprod^p\varphi\]
In this sense, the \(p\)-th exterior power is fully general, and the \(p\)-ary box product is no more general.
\end{rmk}
...
% TODO: matrix of intersection product
...
\dTo<{\widetilde{D}_L}&&\dTo>{D_E}\\
L(\medeprod E^*;\medeprod E^*)&\rTo_{\widetilde{T}}&\medeprod(E^*,E)
\end{diagram}
Interchanging the roles of \(E\) and~\(E^*\), we obtain the map
...
\begin{rmk}[Sylvester-Weinstein-Aronszajn]
...
\begin{quest}
What is the relationship between \(C_p(\varphi_1,\ldots,\varphi_p)\) and \(\Tr_p(\varphi_1,\ldots,\varphi_p)\)?
\end{quest}
...
\end{align*}
If \(\Delta_E\)~is replaced by \(\lambda\Delta_E\), \(\Delta\)~still becomes \(\abs{\lambda}^2\Delta\), so \(\ur{E}\)~has a natural orientation. It is possible to give a more abstract proof of this result which avoids the tedious combinatorics.\footnote{See \url{https://math.stackexchange.com/q/4565568}.}
\end{rmk}
...
\newpage
\section*{Chapter~8}
{\boldmath\textbf{In this chapter, \(E\)~is an \(n\)-dimensional vector space over~\(\Gamma\).}}

\subsection*{\S~3}
\begin{rmk}
Recall that a skew transformation \(\varphi:E\to E\) naturally corresponds to a skew \emph{bilinear form} \(\Phi:E\times E\to\Gamma\) given by
\[\Phi(x,y)=\iprod{x}{\varphi y}\tag{1}\]
If we ``curry''~\(\Phi\), we obtain the corresponding linear map \(\curry{\Phi}:E\to E^*\) given by
\[\sprod{\curry{\Phi}x}{y}=\Phi(x,y)\tag{2}\]
The graph of~\(\varphi\), when \(\varphi\)~is viewed as a form in this way, is naturally represented as a \emph{subspace} of \(E^*\dsum E\):
\[\Graph(\varphi)=\{\,\curry{\Phi}x\dsum x\mid x\in E\,\}\tag{3}\]
Now by (1) and~(2), skew-symmetry of~\(\varphi\), and the definition~\(\sigma\),
\[\sprod{\curry{\Phi}x}{y}=\iprod{x}{\varphi y}=\iprod{-\varphi x}{y}=\sprod{-\sigma\varphi x}{y}\tag{4}\]
By nondegeneracy of the scalar product, (4)~implies
\[\curry{\Phi}=-\sigma\varphi\qquad\text{and}\qquad\varphi=-\sigma^{-1}\curry{\Phi}\tag{5}\]
By the definition of~\(\tau\), and~(5),
\begin{align*}
\tau(\curry{\Phi}x\dsum x)&=(\sigma x+\curry{\Phi}x)\dsum(x-\sigma^{-1}\curry{\Phi}x)\\
	&=(\sigma x-\sigma\varphi x)\dsum(x+\varphi x)\\
	&=\sigma(\iota-\varphi)x\dsum(\iota+\varphi)x\tag{6}
\end{align*}
Since \(\varphi\)~is skew, \(\iota+\varphi\) is invertible, and writing \(v=(\iota-\varphi)x\) and \(u=(\iota+\varphi)x\), we have
\[v=(\iota-\varphi)(\iota+\varphi)^{-1}u=\Cayley(\varphi)u\tag{7}\]
where \(\Cayley(\varphi):E\to E\) is the \emph{Cayley transform} of~\(\varphi\). If we define
\[\Graph(\Cayley(\varphi))=\{\,\sigma(\Cayley(\varphi)u)\dsum u\mid u\in E\,\}\tag{8}\]
then it follows from (3) and (6)--(8) that
\[\boxed{\tau(\Graph(\varphi))=\Graph(\Cayley(\varphi))}\tag{9}\]
So \(\tau\)~is an intrinsic, coordinate-free, geometric version of the Cayley transform operating on graphs in the space \(E^*\dsum E\).
\end{rmk}

\begin{rmk}
In addition to the natural inner product in \(E^*\dsum E\), there is another inner product which is induced by the inner product in~\(E\):\footnote{See subsection~1.25 for the definition of \emph{inner product}.}
\[\sssprod{x^*\dsum x}{y^*\dsum y}=\iprod{x}{y}-\iprod{x^*}{y^*}\]
We have
\begin{align*}
\ssprod{\tau(x^*\dsum x)}{\tau(y^*\dsum y)}&=\ssprod{\tau^2(x^*\dsum x)}{y^*\dsum y}\\
	&=2\ssprod{\sigma x\dsum-\sigma^{-1}x^*}{y^*\dsum y}\\
	&=2\bigl[\sprod{\sigma x}{y}-\sprod{y^*}{\sigma^{-1}x^*}\bigr]\\
	&=2\bigl[\iprod{x}{y}-\iprod{x^*}{y^*}\bigr]\\
	&=2\sssprod{x^*\dsum x}{y^*\dsum y}
\end{align*}
Therefore \(\tau\)~is (up to the constant factor) an \emph{isometry} between the natural and induced inner products in \(E^*\dsum E\).

If \(\varphi:E\to E\) is skew, then \(\Graph(\varphi\))~is totally isotropic under the natural inner product since
\[\ssprod{\curry{\Phi}x\dsum x}{\curry{\Phi}x\dsum x}=2\sprod{\curry{\Phi}x}{x}=2\iprod{\varphi x}{x}=0\]
Since \(\Cayley(\varphi)\)~is orthogonal, \(\Graph(\Cayley(\varphi))\)~is also totally isotropic under the induced inner product since
\[\sssprod{\sigma(\Cayley(\varphi)u)\dsum u}{\sigma(\Cayley(\varphi)u)\dsum u}=\iprod{u}{u}-\iprod{\Cayley(\varphi)u}{\Cayley(\varphi)u}=0\]
\end{rmk}

\begin{rmk}
For a skew transformation \(\varphi:E\to E\), the map~\(\tau_{\eprod}\) provides a ``bridge'' between the tensor representations of~\(\varphi\) in \(E^*\tprod E\) and~\(\bigwedge^2 E\).

In the proof of Theorem~8.3.1, this bridge is crossed---from (8.9) to~(8.13)---to obtain more detailed information about the characteristic coefficients of a skew transformation.
\end{rmk}

\subsection*{\S~4}
\begin{rmk}
In the proof of Proposition~8.4.1, we see that the following diagram commutes for an isometry \(\tau\):
\begin{diagram}
\Sk(E)										&\rTo^{\Psi_E}_{\iso}	&\medeprod^2 E\\
\dTo<{\tau\after(-)\after\tau^{-1}}>{\iso}	&						&\dTo>{\tau_{\eprod}}<{\iso}\\
\Sk(E)										&\rTo_{\Psi_E}^{\iso}	&\medeprod^2 E
\end{diagram}
This shows that \(\tau_{\eprod}\) performs conjugation by~\(\tau\) for tensor representations of skew transformations.
\end{rmk}

\begin{quest}
For skew transformations, what is the relationship between mixed characteristic coefficients and mixed Pfaffians?
\end{quest}

\subsection*{\S~5}
\begin{rmk}
We see that the following diagrams commute:
\begin{diagram}
\Sk(E)							&\rTo^{\Psi_E}_{\iso}			&\medeprod^2 E			&&&\Sk(F)							&\rTo^{\Psi_F}_{\iso}			&\medeprod^2 F\\
\dTo<{i_E\after(-)\after\pi_E}	&								&\dTo>{(i_E)_{\eprod}}	&&&\dTo<{i_F\after(-)\after\pi_F}	&								&\dTo>{(i_F)_{\eprod}}\\
\Sk(E\dsum F)					&\rTo_{\Psi_{E\dsum F}}^{\iso}	&\medeprod^2(E\dsum F)	&&&\Sk(E\dsum F)					&\rTo_{\Psi_{E\dsum F}}^{\iso}	&\medeprod^2(E\dsum F)
\end{diagram}
These show that \((i_E)_{\eprod}\) and~\((i_F)_{\eprod}\) perform injection into direct sums for tensor representations of skew transformations.

In~(8.14), we then see that \emph{the tensor representation of a (direct) sum of skew transformations is the sum of the tensor representations of the transformations}:
\begin{diagram}
\Sk(E)\times\Sk(F)	&\rTo^{\Psi_E\times\Psi_F}	&\medeprod^2 E\times\medeprod^2 F	&\rTo^{(i_E)_{\eprod}\times(i_F)_{\eprod}}	&\medeprod^2(E\dsum F)\times\medeprod^2(E\dsum F)\\
\dTo<{\dsum}		&							&									&											&\dTo>+\\
\Sk(E\dsum F)		&							&\rTo_{\Psi_{E\dsum F}}^{\iso}		&											&\medeprod^2(E\dsum F)
\end{diagram}
\end{rmk}

\subsection*{\S~7}
\begin{rmk}
If \(E\)~is complex and \(\varphi:\ur{E}\to\ur{E}\) is \(\R\)-linear, then the anticommutator \(\acomm{\ir}{\varphi}:E\to E\) defined by
\[\acomm{\ir}{\varphi}=\ir\after\varphi+\varphi\after\ir\]
is \(\C\)-linear since it is \(\R\)-linear and
\[\acomm{\ir}{\varphi}\after\ir=\ir\after\acomm{\ir}{\varphi}\]
The commutator \(\comm{\ir}{\varphi}:E\to E\) defined by
\[\comm{\ir}{\varphi}=\ir\after\varphi-\varphi\after\ir\]
is conjugate \(\C\)-linear. Note
\[\varphi=\tfrac{1}{2i}\bigl(\,\acomm{\ir}{\varphi}+\comm{\ir}{\varphi}\,\bigr)\]
If \(\varphi:E\to E\) is \(\C\)-linear, then \(\acomm{\ir}{\ur{\varphi}}=2i\varphi\) and \(\comm{\ir}{\ur{\varphi}}=0\).
\end{rmk}

\begin{rmk}
If \(E,E^*\) are complex dual spaces with scalar product \(\sprod{-}{-}:E^*\times E\to\C\), then \(\ur{E},\ur{(E^*)}\) are dual with scalar product \(\sprod{-}{-}_{\R}:\ur{(E^*)}\times\ur{E}\to\R\) given by
\[\sprod{x^*}{x}_{\R}=\Re\sprod{x^*}{x}\]
Indeed, clearly \(\sprod{-}{-}_{\R}\) is well-defined and \(\R\)-bilinear. If \(\sprod{x^*}{-}_{\R}=0\), then for any \(x\in\ur{E}\),
\[\Re\sprod{x^*}{x}=\sprod{x^*}{x}_{\R}=0\]
and also
\[\Im\sprod{x^*}{x}=-\sprod{x^*}{\ir x}_{\R}=0\]
so \(\sprod{x^*}{x}=0\). Hence \(\sprod{x^*}{-}=0\) and \(x^*=0\). By symmetry, if \(\sprod{-}{x}_{\R}=0\) then \(x=0\). Therefore \(\sprod{-}{-}_{\R}\) is nondegenerate. We write
\[\ur{E}^*=(\ur{E})^*=\ur{(E^*)}\]
\end{rmk}

\begin{rmk}
If \(E,E^*\) and \(F,F^*\) are pairs of complex dual spaces and \(\varphi:E\to F\) and \(\varphi^*:E^*\from F^*\) are complex dual maps, then \(\ur{\varphi}:\ur{E}\to\ur{F}\) and \(\ur{(\varphi^*)}:\ur{E}^*\from\ur{F}^*\) are dual since
\[\sprod{\ur{(\varphi^*)}y^*}{x}_{\R}=\Re\sprod{\varphi^*y^*}{x}=\Re\sprod{y^*}{\varphi x}=\sprod{y^*}{\ur{\varphi}x}_{\R}\]
We write
\[\ur{\varphi}^*=(\ur{\varphi})^*=\ur{(\varphi^*)}\]
\end{rmk}

\begin{rmk}
If \(E\)~is complex with Hermitian inner product \(\iprod{-}{-}_H:E\times E\to\C\) (that is, a unitary space), and \(\ur{E}\)~has the inner product \(\iprod{-}{-}:\ur{E}\times\ur{E}\to\R\) given by
\[\iprod{x}{y}=\Re\iprod{x}{y}_H\]
then if \(\varphi:E\to E\) is skew-Hermitian, also \(\ur{\varphi}:\ur{E}\to\ur{E}\) is skew-symmetric since
\[\adj{\ur{\varphi}}=\ur{\adj{\varphi}}=\ur{(-\varphi)}=-(\ur{\varphi})\]
Therefore it makes sense to talk about \(\Psi_{\ur{E}}(\ur{\varphi})\in\bigwedge^2\ur{E}\)---which in the book is confusingly denoted~\(\Psi_E(\varphi)\).
\end{rmk}

\begin{rmk}
If \(E\)~is again a unitary space, recall that there is a \emph{conjugate-linear} Riesz isomorphism \(\sigma_H:E\to E^*\) with
\[\sprod{\sigma_H x}{y}=\conj{\iprod{x}{y}_H}\]
For \(x,y\in\ur{E}\),
\[\conj{\iprod{x}{y}_H}=\Re\iprod{x}{y}_H-i\Im\iprod{x}{y}_H=\iprod{x}{y}-i\Im\iprod{x}{y}_H\]
and
\[\iprod{x}{iy}_H=-i\iprod{x}{y}_H=\Im\iprod{x}{y}_H-i\Re\iprod{x}{y}_H\]
so that
\[\iprod{x}{\ir y}=\Re\iprod{x}{iy}_H=\Im\iprod{x}{y}_H\]
and hence
\[\sprod{\sigma_H x}{y}=\iprod{x}{y}-i\iprod{x}{\ir y}\]
This shows that the map \(\sigma:\ur{E}\to E^*\) in the book is given by \(\sigma=\iota_{\R,\C}\after\ur{(\sigma_H)}\), where \(\ur{(\sigma_H)}:\ur{E}\to\ur{E^*}\) is the underlying real map of the Riesz isomorphism and \(\iota_{\R,\C}:\ur{E^*}\to E^*\) is the identity map. Note
\[\sprod{\ur{(\sigma_H)}x}{y}_{\R}=\iprod{x}{y}\]
so \(\ur{(\sigma_H)}\)~is also the Riesz isomorphism for the underlying real spaces. Note also that (8.16)~follows from conjugate-linearity of~\(\sigma_H\).

The map \(\sigma\)~is used in order to avoid the conjugate-linearity of~\(\sigma_H\), which does not play as well with the multilinearity of tensor and exterior algebra.
\end{rmk}

\begin{rmk}
If \(E\)~is again a unitary space and \(\varphi:E\to E\) is skew-Hermitian, then for \(z=\Psi_{\ur{E}}(\ur{\varphi})\) and \(J=\Psi_{\ur{E}}(\ir)\) and \(0\le r\le n\),
\begin{align*}
\iprod{z^r}{z^r}&=C_{2r}(\ur{\varphi})&&\text{by Theorem 8.3.1}\\
	&=\sum_{p+q=2r}C_p(\varphi)\conj{C_q(\varphi)}&&\text{by Proposition 7.9.1}\\
	&=\sum_{p+q=2r}i^p\iprod{z^p}{J^p}\conj{i^q\iprod{z^q}{J^q}}&&\text{by Theorem 8.7.1}\\
	&=(-1)^r\sum_{p+q=2r}\iprod{z^p}{J^p}\iprod{z^q}{(-J)^q}&&
\end{align*}
In particular for \(r=n\),
\[\iprod{z^n}{z^n}=\iprod{z^n}{J^n}^2\]
Since \(\det(\ur{\varphi})=\iprod{z^n}{z^n}\), and \(\Pf(\ur{\varphi})=\iprod{z^n}{J^n}\) by the corollary to Theorem~8.7.1, this is consistent with
\[\det(\ur{\varphi})=\Pf(\ur{\varphi})^2=(-1)^n\det(\varphi)^2\]
\end{rmk}
...
\subsection*{\S~10}
\begin{rmk}
It follows from the lemma to Proposition~9.7.1 that \(\medsmprod^p E\)~has a basis of elements of the form~\(h^p\) for \(h\in E\). Indeed if this were not the case, then the orthogonal complement in~\(\medsmprod^p E^*\) of the span of such elements would contain an element \(u^*\ne 0\) such that
\[i(h)^pu^*=p!\,i(h^p)u^*=p!\,\sprod{u^*}{h^p}=0\]
for all \(h\in E\), contradicting the lemma.
\end{rmk}
...
\bibitem{greub3} Greub, W. \textit{Multilinear Algebra}, 1st~ed. Springer, 1967.
\bibitem{greub4} Greub, W. and Vanstone, J. ``A Basic Identity in Mixed Exterior Algebra.'' \textit{Linear and Multilinear Algebra}. Gordon and Breach, 1987.
\end{thebibliography}
\end{document}