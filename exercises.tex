...
\end{proof}
\noindent In fact the Poincar\'e isomorphisms \(D_e\) and~\(D^e\) are anti-isomorphisms between the lattices of decomposable elements of \(\medeprod E\) and~\(\medeprod E^*\) under divisibility,\footnote{See subsection~6.30 of~\cite{greub3}.} so this result follows from surjectivity of \(D^1:E^*\to\medeprod^{n-1}E\).
...
for all \(h,x_i\in E\). This identity characterizes~\(\ad(\varphi)\).\footnote{See also the remark in chapter~IV, \S~2 of~\cite{greub1} above.}
\end{rmk}
