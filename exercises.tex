...
\subsection*{\S~7}
\begin{rmk}
If \(E\)~is complex with Hermitian inner product \(\iprod{-}{-}_H:E\times E\to\C\), and \(E_{\R}\)~is the underlying real space with inner product \(\iprod{-}{-}:E_{\R}\times E_{\R}\to\R\) given by
\[\iprod{x}{y}=\Re\iprod{x}{y}_H\qquad(x,y\in E_{\R})\]
then if \(\varphi:E\to E\) is skew-Hermitian, the underlying map \(\varphi_{\R}:E_{\R}\to E_{\R}\) is skew-symmetric since
