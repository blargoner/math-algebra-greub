...
\[T_E(\eprods{z}{1}{k})=(-1)^{k(k-1)/2}T_Ez_1\bprod\cdots\bprod T_Ez_k\qquad(z_i\in E^*\tprod E)\]
In particular
...
the matrix \(M(D_L(\medeprod^{n-p}\varphi))\) consists of the components of~\(D_Ez^{n-p}\), so is the transpose of the matrix of signed \((n-p)\times(n-p)\) minors of~\(M\) complementary to the minors in~\(M(\medeprod^p\varphi)\).\footnote{See the remark in chapter~6, \S~13 above.} By the corollary to Proposition~7.2.1,
\[M(\medeprod^p\varphi)M(D_L(\medeprod^{n-p}\varphi))=\det M\mult I\]
which, by the above results, expresses the classical Laplace expansion of~\(\det M\) by minors from any \(p\)~rows of~\(M\) and their corresponding cofactors.\footnote{See also problem~7.9.9.}
\end{rmk}
