...
\begin{rmk}
If \(E\)~is \emph{finite-dimensional}, we can generalize the definition of the box product. For linear transformations \(\varphi\) and~\(\psi\) of~\(\medeprod E\), we define
...
\begin{rmk}
\(\Ad(\varphi_1,\ldots,\varphi_{n-1})\) does not depend upon the choice of~\(\Delta\), and therefore only depends upon \(\varphi_1,\ldots,\varphi_{n-1}\).
\end{rmk}
