...
\subsection*{\S~15}
\begin{rmk}
The elements of~\(\medeprod(E\dsum F)\) are sums of wedge products of elements from~\(E\dsum F\), which by bilinearity of the wedge product expand to sums of wedge products of elements from \(E\) and elements from~\(F\). By anticommutativity of the wedge product, the elements from~\(E\) in any product can be moved to the left of the elements from~\(F\) with at most a change in sign, resulting in the wedge product of an element from~\(\medeprod E\) and an element from~\(\medeprod F\). This leads to the natural isomorphism
...
\begin{diagram}
\medeprod E_1\stprod\medeprod E_2				&					&\rTo^{g}								&							&\medeprod E\\
\dTo<{(\rho_1)_{\eprod}\tprod(\rho_2)_{\eprod}}	&					&										&							&\dTo>{\rho_{\eprod}}\\
\medeprod\proj{E_1}\stprod\medeprod\proj{E_2}	&\rTo_{f}^{\iso}	&\medeprod(\proj{E_1}\dsum\proj{E_2})	&\rTo_{\psi_{\eprod}}^{\iso}&\medeprod\proj{E}
\end{diagram}
Here \(f\) and~\(g\) are defined as in~(5.42) (although \(g\)~is not necessarily injective), the \(\rho\)'s are canonical projections, and
\[\psi=[\proj{i_1},\proj{i_2}]:\proj{E_1}\dsum\proj{E_2}\to\proj{E}\]
is the natural isomorphism described in chapter~II, \S~4 of~\cite{greub1} above.
\end{exer}
\begin{proof}
...
\begin{align*}
(\rho_{\eprod}\after g)(u_1\tprod u_2)&=\rho_{\eprod}\bigl((i_1)_{\eprod}(u_1)\eprod(i_2)_{\eprod}(u_2)\bigr)\\
	...
	&=\psi_{\eprod}\bigl[f\bigl((\rho_1)_{\eprod}(u_1)\tprod(\rho_2)_{\eprod}(u_2)\bigr)\bigr]\\
	&=\bigl[\psi_{\eprod}\after f\after\bigl((\rho_1)_{\eprod}\tprod(\rho_2)_{\eprod}\bigr)\bigr](u_1\tprod u_2)
\end{align*}
