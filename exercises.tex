...
If \(\varphi:E\to E\) is skew, then \(\Graph(\varphi\))~is \emph{totally isotropic} under the natural inner product since
\[\ssprod{\curry{\Phi}x\dsum x}{\curry{\Phi}x\dsum x}=2\sprod{\curry{\Phi}x}{x}=2\iprod{\varphi x}{x}=0\]
Since \(\tau\)~is an isometry, it follows that \(\Graph(\Cayley(\varphi))\) is totally isotropic under the induced inner product, so
\begin{align*}
0&=\sssprod{\sigma(\Cayley(\varphi)u)\dsum u}{\sigma(\Cayley(\varphi)u)\dsum u}\\
	&=\iprod{u}{u}-\iprod{\Cayley(\varphi)u}{\Cayley(\varphi)u}
\end{align*}
This shows that \(\Cayley(\varphi)\)~is orthogonal, as expected.
\end{rmk}
