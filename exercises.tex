...
This shows that the map \(\sigma:\ur{E}\to E^*\) in the book is given by \(\sigma=\iota_{\R,\C}\after\ur{(\sigma_H)}\), where \(\ur{(\sigma_H)}:\ur{E}\to\ur{E^*}\) is the underlying real map of the Riesz isomorphism and \(\iota_{\R,\C}:\ur{E^*}\to E^*\) is the identity map.  Note (8.16)~follows from conjugate-linearity of~\(\sigma_H\).
\end{rmk}
