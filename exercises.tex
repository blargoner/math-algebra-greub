...
\[D_e(\eprods{x}{1}{p})=\sum_{<}(-1)^{\sum_{i=1}^p(\nu_i-i)}\det\sprod{e^{*\nu_j}}{x_i}e^{\nu_{p+1}\cdots\nu_n}\]
...
\begin{exer}[12]
If \(\varphi:E^{n-1}\to F\) is an \((n-1)\)-linear skew-symmetric map, then there is a unique map \(\chi:E^*\to F\) with
