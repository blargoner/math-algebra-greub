...
Taking \(x=y\in F\) and multiplying both sides by~\(a\), it follows that \(\sigma_0 y=0\) for all \(y\in F\), so \(\sigma y=a\tprod\sigma_1 y\) for all \(y\in F\). A simple computation then shows that \(y\eprod\sigma_1 y=0\) for all \(y\in F\), and hence in particular for \(y\in F'\).
