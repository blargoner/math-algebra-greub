...
In terms of matrices, this identity relates the minors of a matrix to the minors of its adjoint matrix.\footnote{See problem~7.9.11.}
\end{rmk}
...
\begin{rmk}
Let \(e^*\in\medeprod^n E^*\) correspond to~\(\Delta\). By Proposition~7.4.1(2),
\begin{align*}
\Delta(h,\varphi x_1,\ldots,\varphi x_{n-1})&=\sprod{e^*}{h\eprod\eprods{\varphi x}{1}{n-1}}\\
	&=\sprod{D_e h}{\medeprod^{n-1}\varphi(\eprods{x}{1}{n-1})}\\
	&=\sprod{(D_e^{-1}\after\medeprod^{n-1}\varphi^*\after D_e)h}{(D^e)^{-1}(\eprods{x}{1}{n-1})}\\
	&=\sprod{\ad(\varphi)h}{D_e^*(\eprods{x}{1}{n-1})}\\
	&=\sprod{D_e\ad(\varphi)h}{\eprods{x}{1}{n-1}}\\
	&=\sprod{e^*}{\ad(\varphi)h\eprod\eprods{x}{1}{n-1}}\\
	&=\Delta(\ad(\varphi)h,x_1,\ldots,x_{n-1})
\end{align*}
This identity characterizes~\(\ad(\varphi)\).\footnote{See also the remark in chapter~IV, \S~2 of~\cite{greub1} above.}
\end{rmk}
