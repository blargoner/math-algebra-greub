...
\begin{rmk}
If \(E,E^*\) and \(F,F^*\) are pairs of complex dual spaces and \(\varphi:E\to F\) and \(\varphi^*:E^*\from F^*\) are complex dual maps, then the underlying real maps \(\ur{\varphi}:\ur{E}\to\ur{F}\) and \(\ur{(\varphi^*)}:\ur{E}^*\from\ur{F}^*\) are dual since
...
then if \(\varphi:E\to E\) is skew-Hermitian, the underlying real map \(\ur{\varphi}:\ur{E}\to\ur{E}\) is skew-symmetric since
