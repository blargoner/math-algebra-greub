...
\usepackage{amsmath,amssymb,amsthm,enumitem,fourier,diagrams,tikz-cd,stmaryrd}
...
and extend the definition to~\(\medeprod E\) through bilinearity, then the resulting algebra \((\tilde{\medeprod}E,\eprodf)\) is also an exterior algebra over~\(E\).
\end{exer}
\begin{proof}
Define \(f:\medeprod E\to\tilde{\medeprod}E\) by \(f(u)=p!u\) for \(u\in\medeprod^p E\). Then clearly \(f\)~is a linear isomorphism homogeneous of degree zero, and
\[f(u\eprod v)=(p+q)!\,u\eprod v=\frac{(p+q)!}{p!q!}(p!u)\eprod(q!v)=f(u)\eprodf f(v)\]
for \(u\in\medeprod^p E\) and \(v\in\medeprod^q E\), so \(f\)~is a graded algebra isomorphism. It follows that \((\tilde{\medeprod}E,\eprodf)\) satisfies the universal property of an exterior algebra over~\(E\).
\end{proof}
\begin{rmk}
If \(E,E^*\) are dual spaces, then \(\tilde{\medeprod}E,\tilde{\medeprod}E^*\) are dual graded algebras under the scalar product induced by~\(f\), which is just the scalar product in~(5.10) with \(\eprod\) replaced by~\(\eprodf\).
\end{rmk}
...
\end{exer}
\begin{proof}
The right-hand side is a nonzero graded subalgebra of~\(\medeprod E\) with grade \(1\) subspace~\(F\) which is closed under~\(i(h^*)\) for all \(h^*\in E^*\) by~(5.38), so the result follows from Proposition~5.28.2.
\end{proof}
...
so
\[i_A(h)\Phi=i_1(h)\Phi\]
as desired. This redefinition is required to make~\(i_A(h)\) an antiderivation in the algebra~\(A^{\adot}(E)\) of skew-symmetric multilinear functions (under the definition of Grassmann product in subsection~5.30),\footnote{See Proposition~5.32.1.} although under this redefinition \(i_A(h)\)~is not an antiderivation in the algebra~\(T^{\tdot}(E)\) of multilinear functions.\footnote{See subsection~3.19.}
\end{rmk}
